...
class GiftLogDao: BaseDaoInterface<GiftLog> {
    ...
    override fun getAll(key: String, value: Any?): List<GiftLog> {
        ...
        if (key == "" || value == null)
            return mongoTemplate.findAll(GiftLog::class.java)

        val query = Query(Criteria.where(key).`is`(value))

        return mongoTemplate.find(query,GiftLog::class.java)
    }

    override fun insert(log: GiftLog) {
    ...
    override fun findByKey(key: String, value: Any): GiftLog? {
        ...
        return mongoTemplate.findOne(query,GiftLog::class.java)
    }
    ...
    override fun updata(_id: String, data: HashMap<String, Any>): UpdateResult {
       ...
       data.forEach { key, value ->
        ...
        return mongoTemplate.updateMulti(query, update!!, GiftLog::class.java)
    ...
    override fun delete(_id: String): DeleteResult {
        val criteria = Criteria.where("_id").`is`(_id)
        val query = Query(criteria)
        return mongoTemplate.remove(query,GiftLog::class.java)
    }
}